import { Check, Copy, FileAudio, Loader2, Mic, Square, Trash2, Upload } from 'lucide-react'
    ...
    <section className="panel pg-modality-panel">
      <div className="pg-modality-settings">
        <div className="pg-settings-group">
          ...
          <select id="pg-tx-model" className="pg-select" value={tx.model} onChange={(e) => tx.setModel(e.target.value)}>
      ...
      <div className="pg-tx-actions">
        ...
        >
          ...
          {tx.loading ? 'transcribing…' : 'transcribe'}
        </button>
      </div>
      ...
      {!tx.loading && !tx.file && tx.transcript == null && !tx.error ? (
        <div className="pg-empty">
          <Mic className="pg-empty-icon" strokeWidth={1.25} />
          <span>Upload or record audio for transcription</span>
        </div>
      ) : null}
    </section>
  )
