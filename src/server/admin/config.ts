...
async function fetchSchema(): Promise<Record<string, unknown>> {
...
export async function validateAgainstSchema(content: string): Promise<ValidationResult> {
  ...
  try {
    const { $schema, ...schema } = await fetchSchema()
...
export function writeConfig(content: string, expectedModifiedAt: number): { written: boolean; conflict: boolean } {
  const p = configPath()
  const stat = statSync(p)
