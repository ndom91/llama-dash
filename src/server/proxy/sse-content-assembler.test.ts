...
describe('SseContentAssembler', () => {
  it('splits OpenAI reasoning_content and content deltas', () => {
    ...
    a.feed('data: [DONE]\n\n')
    expect(a.result()).toEqual({ reasoning: 'think hard', response: 'Hi!' })
  })
  ...
  it('handles Anthropic thinking and text deltas', () => {
    ...
    expect(a.result()).toEqual({ reasoning: 'plan', response: 'ans' })
  })

  it('returns nulls when empty', () => {
    expect(new SseContentAssembler().result()).toEqual({ reasoning: null, response: null })
  })
})
