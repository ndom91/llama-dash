...
- **Dashboard auth** — Better Auth username/password session gate for the UI and `/api/*` with first-visit signup; `/v1/*` proxy auth stays API-key based.
...
- `src/server/auth.ts` — Better Auth setup for dashboard username/password sessions; protects UI and `/api/*`, not `/v1/*`. Signup is only allowed while no dashboard user exists.
